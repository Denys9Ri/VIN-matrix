...
def recommendation_section_html(visit, doc_type):
    ...
    for item in items:
        ...
        description_html = ''
        description = str(recommendation.description or '').strip()
        is_auto_diagnostic_description = description.startswith('Виявлено під час діагностики. Розділ:')
        if description and not is_auto_diagnostic_description:
            description_html = f"<div class='rec-description'>{nl2br(description)}</div>"
        meta_html = ''
        if meta:
