...
def pay_type_label(value, company=None):
    key = value or 'cash'
    if company:
        try:
            seed_company_options(company)
            option = CompanyOption.objects.filter(company=company, group='payment_type', key=key, is_active=True).first()
            if option:
                return option.label
        except Exception:
            pass
    return FALLBACK_PAYMENT_TYPES.get(key, key or 'Готівка')


def payment_type_map(company):
    try:
        seed_company_options(company)
        return {item.key: item.label for item in CompanyOption.objects.filter(company=company, group='payment_type', is_active=True)}
    except Exception:
        return FALLBACK_PAYMENT_TYPES.copy()


def rows_for_visit(visit_id, company=None):
    labels = payment_type_map(company) if company else FALLBACK_PAYMENT_TYPES
    ...
    return [{
        ...
        'amount': f(r[1]),
        'payment_type': r[2],
        'payment_type_label': labels.get(r[2], pay_type_label(r[2], company)),
        'payment_purpose': r[3],
        'comment': r[4] or '',
        'created_at': r[5]
    } for r in rows]
...
def add_payment(visit, user, amount, payment_type='cash', purpose='partial', comment=''):
    ...
    with connection.cursor() as cursor:
        cursor.execute(
            ...
            [visit.company_id, visit.id, amount, payment_type, purpose or 'partial', comment or '', timezone.now(), user.id if user and user.is_authenticated else None],
        )
        payment_id = cursor.fetchone()[0]
    finance = sync_status(visit)
    is_final = purpose == 'final' or f(finance.get('debt_amount')) <= 0
    label = pay_type_label(payment_type, visit.company)
    log_activity(
        ...
        description=f"{money_text(amount)} · {label} · залишок боргу {money_text(finance.get('debt_amount'))}" + (f" · {comment}" if comment else ''),
        old_value=None,
...
class VisitPaymentListView(APIView):
    ...
    def get(self, request):
        ...
        if not visit:
            return Response({'results': []})
        return Response({'results': rows_for_visit(visit.id, company), 'finance': totals_for_visit(visit), 'payment_types': payment_type_map(company)})
    ...
    def post(self, request):
        ...
        return Response({'id': payment_id, 'payments': rows_for_visit(visit.id, company), 'finance': finance, 'visit_id': visit.id, 'payment_types': payment_type_map(company)})
...
class VisitMarkPaidView(APIView):
    ...
    def post(self, request, pk):
        ...
        return Response({'payments': rows_for_visit(visit.id, company), 'finance': sync_status(visit), 'visit_id': visit.id, 'payment_types': payment_type_map(company)})
