...
const defaultChecklist = diagnosticItems.reduce((acc, [key]) => ({ ...acc, [key]: { status: 'not_checked', note: '' } }), {});
...
export default function VisitWorkflowPanel({ selectedGroup, lastVisit, initialActive = 'acceptance', standalone = false }) {
  ...
  const saveAcceptance = async (event, finalStatus = acceptance.status) => {
    ...
    try {
      ...
      setAcceptance((prev) => ({ ...prev, ...(response.data || {}), status: response.data?.status || finalStatus }));
    } catch {
  ...
  const saveDiagnostic = async (event, finalStatus = diagnostic.status) => {
    ...
    try {
      ...
      setDiagnostic((prev) => ({ ...prev, ...(response.data || {}), checklist: response.data?.checklist || prev.checklist, status: response.data?.status || finalStatus }));
    } catch {
  ...
  const updateDiagnosticItem = (key, field, value) => {
    setDiagnostic((prev) => ({
      ...prev,
      checklist: { ...prev.checklist, [key]: { ...(prev.checklist?.[key] || {}), [field]: value } },
    }));
  };
  ...
  return (
    <div className={standalone ? 'bg-white rounded-3xl sm:rounded-2xl border border-slate-200 p-5 sm:p-4 shadow-sm min-w-0' : 'bg-white rounded-3xl sm:rounded-2xl border border-slate-200 p-5 sm:p-4 shadow-sm min-w-0'}>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
        ...
        <span className="rounded-lg border border-slate-200 bg-slate-50 px-2 py-1 text-[10px] font-black uppercase text-slate-500">{loading ? 'Завантаження...' : 'Готово'}</span>
      </div>

      <div className="overflow-x-auto -mx-1 px-1 mb-4">
        <div className="flex gap-2 min-w-max pb-1">
          {tabs.map(([key, label, Icon]) => <button key={key} type="button" onClick={() => setActive(key)} className={`flex items-center gap-2 px-4 py-3 rounded-2xl text-xs font-black uppercase border whitespace-nowrap transition-all ${active === key ? 'bg-blue-600 text-white border-blue-600 shadow-md shadow-blue-100' : 'bg-slate-50 text-slate-500 border-slate-100 hover:bg-slate-100'}`}><Icon size={15} /> {label}</button>)}
        </div>
      </div>
        ...
        <form onSubmit={(event) => saveAcceptance(event)} className="space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <div className="bg-slate-50 border border-slate-100 rounded-2xl p-3 min-w-0">
              ...
              <input type="number" value={acceptance.mileage} onChange={(event) => setAcceptance({ ...acceptance, mileage: event.target.value })} className="mt-1 w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-sm font-bold outline-none focus:border-blue-500" />
            </div>
            <div className="bg-slate-50 border border-slate-100 rounded-2xl p-3 min-w-0">
              <label className="text-[10px] font-black uppercase text-slate-400">Паливо</label>
              <select value={acceptance.fuel_level} onChange={(event) => setAcceptance({ ...acceptance, fuel_level: event.target.value })} className="mt-1 w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-sm font-bold outline-none focus:border-blue-500">
                {fuelOptions.map((option) => <option key={option} value={option}>{option}</option>)}
              </select>
            </div>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
            <TextArea label="Скарга клієнта" value={acceptance.customer_complaint} onChange={(value) => setAcceptance({ ...acceptance, customer_complaint: value })} placeholder="Що турбує клієнта: шум, стук, не заводиться..." />
            <TextArea label="Пошкодження кузова" value={acceptance.damages} onChange={(value) => setAcceptance({ ...acceptance, damages: value })} placeholder="Подряпини, вмʼятини, сколи, стан ЛФП" />
            <TextArea label="Салон / речі в авто" value={acceptance.interior_note} onChange={(value) => setAcceptance({ ...acceptance, interior_note: value })} placeholder="Документи, ключі, килимки, особисті речі" />
            <TextArea label="Зовнішній стан / примітка" value={acceptance.exterior_note} onChange={(value) => setAcceptance({ ...acceptance, exterior_note: value })} placeholder="Стан авто при прийманні" />
          </div>
          <TextArea label="Загальна примітка" value={acceptance.note} onChange={(value) => setAcceptance({ ...acceptance, note: value })} placeholder="Що важливо не забути" rows={2} />
          <div className="flex flex-col sm:flex-row gap-2 justify-end">
            ...
            <button type="button" onClick={(event) => saveAcceptance(event, 'completed')} disabled={savingAcceptance} className="bg-emerald-600 text-white rounded-xl px-4 py-3 text-xs font-black uppercase disabled:opacity-50 flex items-center justify-center gap-2"><CheckCircle2 size={15} /> Акт готовий</button>
          </div>
        </form>
      )}
        ...
        <form onSubmit={(event) => saveDiagnostic(event)} className="space-y-3">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
            {diagnosticItems.map(([key, label]) => {
              ...
              return <div key={key} className="bg-slate-50 border border-slate-100 rounded-2xl p-3 min-w-0"><div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2"><p className="text-sm font-black text-slate-800">{label}</p><select value={item.status || 'not_checked'} onChange={(event) => updateDiagnosticItem(key, 'status', event.target.value)} className={`border rounded-xl px-3 py-2 text-xs font-black uppercase outline-none ${statusClass[item.status] || statusClass.not_checked}`}>{statusOptions.map(([value, optionLabel]) => <option key={value} value={value}>{optionLabel}</option>)}</select></div><input value={item.note || ''} onChange={(event) => updateDiagnosticItem(key, 'note', event.target.value)} placeholder="Коментар майстра" className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-sm font-bold outline-none focus:border-blue-500" /></div>;
            })}
          </div>
          <div className="bg-amber-50 border border-amber-100 rounded-2xl p-3 flex gap-2 text-sm font-semibold text-amber-700"><AlertTriangle size={18} className="shrink-0" /> Якщо є критичні пункти, їх потім зручно перенести в рекомендації клієнту.</div>
          ...
          <div className="flex flex-col sm:flex-row gap-2 justify-end">
            <button type="submit" disabled={savingDiagnostic} className="bg-slate-900 text-white rounded-xl px-4 py-3 text-xs font-black uppercase disabled:opacity-50 flex items-center justify-center gap-2"><Save size={15} /> {savingDiagnostic ? 'Зберігаю...' : 'Зберегти чернетку'}</button>
            <button type="button" onClick={(event) => saveDiagnostic(event, 'completed')} disabled={savingDiagnostic} className="bg-emerald-600 text-white rounded-xl px-4 py-3 text-xs font-black uppercase disabled:opacity-50 flex items-center justify-center gap-2"><CheckCircle2 size={15} /> Діагностика готова</button>
          </div>
        </form>
      )}
    </div>
...
function TextArea({ label, value, onChange, placeholder, rows = 3 }) {
  return <div className="bg-slate-50 border border-slate-100 rounded-2xl p-3 min-w-0"><label className="text-[10px] font-black uppercase text-slate-400">{label}</label><textarea rows={rows} value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} className="mt-1 w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-sm font-bold outline-none focus:border-blue-500 resize-none" /></div>;
}
