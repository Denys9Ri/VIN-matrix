...
import { documentPhoneText } from '../../utils/companyPhones';
...
function buildDocumentHtml(type, visit, settings, options = {}) {
  ...
  const companyPhones = documentPhoneText(company);
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>${esc(meta.title)} №${esc(visit?.id || '')}</title><style>
    ...
    <header class="top"><div class="brand">${logo ? `<img class="logo" src="${esc(logo)}" alt="logo">` : ''}<div class="company"><h1>${esc(company.name || 'VIN-matrix')}</h1><div class="muted">${esc(company.address || '')}</div><div class="muted">${esc(companyPhones)}</div></div></div><div class="muted req"><b>${esc(date)}</b><br>${nl2br(requisites)}</div></header>
