...
import { LayoutDashboard, CarFront, Briefcase, LineChart, Settings, Users, Search, X, Package } from 'lucide-react';
import axios from 'axios';

const Sidebar = ({ isOpen, closeMenu }) => {
  const [role, setRole] = useState('mechanic'); 
  const [businessType, setBusinessType] = useState('sto'); // ДОДАНО
  ...
  useEffect(() => {
    const fetchUserRole = async () => {
      ...
      try {
        ...
        setBusinessType(response.data.company?.business_type || 'sto'); // Читаємо тип бізнесу
      } catch (error) {
  ...
  const allMenuItems = [
    ...
    { name: visitsName, icon: visitsIcon, path: '/visits' }, // ДИНАМІЧНО
