...
const toneMap = {
  critical: { badge: 'bg-rose-600 text-white', icon: 'bg-rose-50 text-rose-700 border-rose-100', card: 'hover:bg-rose-50' },
  warning: { badge: 'bg-amber-500 text-white', icon: 'bg-amber-50 text-amber-700 border-amber-100', card: 'hover:bg-amber-50' },
  info: { badge: 'bg-blue-600 text-white', icon: 'bg-blue-50 text-blue-700 border-blue-100', card: 'hover:bg-blue-50' },
};
...
function MiniStat({ label, value, tone }) { return <div className="bg-slate-50 border border-slate-100 rounded-2xl p-3"><p className="text-[9px] uppercase font-black text-slate-400">{label}</p><p className={`text-lg font-black ${tone}`}>{value}</p></div>; }
function Empty({ text }) { return <div className="rounded-2xl bg-slate-50 border border-slate-100 p-8 text-center text-sm font-bold text-slate-500">{text}</div>; }
