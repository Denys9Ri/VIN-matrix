...
import { searchVehicleFuels, VEHICLE_FUEL_TYPES } from '../../utils/vehicleFuelCatalog';

export default function VehicleFuelCombobox({ value = '', onChange, label = 'Паливо' }) {
  const inputId = useId();
  const rootRef = useRef(null);
  const [isOpen, setIsOpen] = useState(false);
  const [showAll, setShowAll] = useState(false);
  const visibleFuels = useMemo(
    () => searchVehicleFuels(VEHICLE_FUEL_TYPES, showAll ? '' : value),
    [showAll, value],
  );
    ...
    <div ref={rootRef} className="relative block min-w-0">
      ...
      <div className="relative">
        ...
        <input
          ...
          onKeyDown={(event) => {
            ...
            if (event.key === 'Enter' && isOpen && !showAll && value.trim() && visibleFuels.length) {
              event.preventDefault();
              chooseFuel(visibleFuels[0]);
            }
          }}
          placeholder="Написати або вибрати паливо..."
          autoComplete="off"
        ...
        <button
          ...
          aria-label="Відкрити список типів палива"
      ...
      {isOpen && (
        <div id={`${inputId}-options`} role="listbox" className="absolute left-0 right-0 top-full z-40 mt-1 max-h-64 overflow-y-auto rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl">
          {visibleFuels.length ? visibleFuels.map((fuel) => (
            <button
              key={fuel}
              type="button"
              role="option"
              aria-selected={fuel === value}
              onClick={() => chooseFuel(fuel)}
            ...
            <div className="px-3 py-4 text-center">
              <p className="text-sm font-black text-slate-600">Тип палива не знайдено</p>
