...
import { Clock, Package, Truck, Phone, Car } from 'lucide-react';
...
const VisitCard = ({ visit, onClick, isStore }) => {
  ...
  return (
    <div data-visit-id={visit.id} onClick={onClick} className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm cursor-pointer hover:border-blue-300 hover:shadow-md transition-all group">
      ...
      <div className="mt-3 flex justify-between items-center border-t border-slate-50 pt-2">
        {isStore ? (
          <span className="text-[10px] font-bold text-slate-500 bg-slate-50 px-2 py-1 rounded flex items-center gap-1 truncate">
            ...
            {visit.delivery_type === 'np' ? 'Нова Пошта' : visit.delivery_type === 'courier' ? 'Курʼєр' : 'Самовивіз'}
          </span>
        ) : (
