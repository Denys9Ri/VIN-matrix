...
const formatVisitDate = (visit) => {
  const rawDate = visit?.updated_at || visit?.created_at || visit?.scheduled_datetime || visit?.date;
  if (!rawDate) return '—';
...
const Clients = ({ embedded = false }) => {
  ...
  const selectedSummary = useMemo(() => {
    ...
    return {
      ...
      activeTasks,
      latestServices: allServices.slice(0, 5),
      latestParts: allParts.slice(0, 5),
    };
      ...
      <div className="bg-white w-full sm:w-full max-w-none sm:max-w-5xl rounded-none sm:rounded-3xl shadow-2xl overflow-y-auto sm:overflow-hidden flex flex-col h-[100dvh] sm:h-auto sm:max-h-[calc(100dvh-2rem)] min-w-0 overscroll-contain">
        <div className="p-5 sm:p-4 md:p-6 border-b border-slate-100 bg-slate-50 sm:shrink-0">
          <div className="flex justify-between items-start gap-3 mb-5 sm:mb-4">
            <div className="min-w-0 flex-1">
              ...
              <div className="flex flex-wrap gap-2 mt-3 sm:mt-2 text-sm sm:text-xs font-bold text-slate-500">
                <span className="bg-white border border-slate-200 rounded-xl sm:rounded-lg px-3 sm:px-2 py-2 sm:py-1 flex items-center gap-1"><Phone size={14} className="sm:w-3 sm:h-3" /> {selectedGroup.phone}</span>
                ...
                <span className="bg-white border border-slate-200 rounded-xl sm:rounded-lg px-3 sm:px-2 py-2 sm:py-1 flex items-center gap-1"><Gauge size={14} className="sm:w-3 sm:h-3" /> {formatMileage(selectedSummary?.lastMileage)}</span>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-2 md:gap-3">
            ...
            <MiniStat label="Нагадування" value={selectedSummary?.activeRecs || 0} danger={(selectedSummary?.activeRecs || 0) > 0} />
        ...
        <div className="px-4 sm:px-3 md:px-6 pt-4 bg-white border-b border-slate-100 overflow-x-auto sm:shrink-0">
          <div className="flex gap-2 min-w-max pb-3">
            {clientTabs.map((tab) => {
              ...
              return (
                <button key={tab.key} type="button" onClick={() => setDetailTab(tab.key)} className={`flex items-center gap-2 px-4 sm:px-4 py-3 sm:py-2.5 rounded-2xl text-sm sm:text-xs font-black uppercase whitespace-nowrap transition-all ${active ? 'bg-blue-600 text-white shadow-md shadow-blue-100' : 'bg-slate-50 text-slate-500 hover:bg-slate-100'}`}>
                  <Icon size={16} className="sm:w-[15px] sm:h-[15px]" />
                  {tab.label}
                </button>
              );
            })}
        ...
        <div className="flex-none sm:flex-1 sm:min-h-0 p-4 sm:p-3 md:p-6 overflow-visible sm:overflow-y-auto sm:overscroll-contain space-y-5 sm:space-y-4 min-w-0 pb-8 sm:pb-6">
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 sm:gap-4">
                <InfoBlock title="Останній візит" icon={<Clock3 size={18} className="text-blue-600" />}>
                  {selectedSummary?.lastVisit ? (
                    <VisitMini visit={selectedSummary.lastVisit} onRepeat={openRepeatVisitModal} />
                  ) : (
                    <EmptySmall text="Візитів немає" />
                  )}
                </InfoBlock>
                ...
                <InfoBlock
                  ...
                  action={<button type="button" onClick={openRecommendationModal} className="bg-blue-600 text-white px-4 sm:px-3 py-3 sm:py-2 rounded-xl text-xs sm:text-[10px] font-black uppercase flex items-center justify-center gap-1"><Plus size={14} className="sm:w-[13px] sm:h-[13px]" /> Додати</button>}
                >
                  {activeRecs.length === 0 ? (
                    <EmptySmall text="Активних нагадувань немає" />
                  ) : (
                ...
                <InfoBlock title="Активні задачі" icon={<ListChecks size={18} className="text-indigo-600" />}>
                  {activeTasks.length === 0 ? (
                    <EmptySmall text="Активних задач немає" />
                  ) : (
                    <div className="space-y-3 sm:space-y-2">
          ...
          {detailTab === 'recommendations' && (
            <InfoBlock title="Рекомендації по клієнту" icon={<ClipboardList size={18} className="text-indigo-600" />} action={<button type="button" onClick={openRecommendationModal} className="bg-blue-600 text-white px-4 sm:px-3 py-3 sm:py-2 rounded-xl text-xs sm:text-[10px] font-black uppercase flex items-center justify-center gap-1 hover:bg-blue-700"><Plus size={14} className="sm:w-[13px] sm:h-[13px]" /> Додати</button>}>
              {selectedRecommendations.length === 0 ? <EmptySmall text="По цьому клієнту рекомендацій поки немає." /> : <div className="space-y-3 sm:space-y-2">{selectedRecommendations.map((rec) => <RecommendationRow key={rec.id} rec={rec} onDone={markRecommendationDone} />)}</div>}
            </InfoBlock>
          )}

          {detailTab === 'tasks' && (
            <InfoBlock title="Задачі по клієнту" icon={<ListChecks size={18} className="text-blue-600" />}>
              {selectedTasks.length === 0 ? <EmptySmall text="По цьому клієнту задач поки немає." /> : <div className="space-y-3 sm:space-y-2">{selectedTasks.map((task) => <TaskRow key={task.id} task={task} onDone={markTaskDone} />)}</div>}
            </InfoBlock>
          )}
        ...
        <div className="rounded-2xl bg-white border border-slate-200 p-4 flex flex-col justify-between gap-4 min-w-0">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-sm font-semibold text-slate-500 break-words">
                {warningCount ? `Активних нагадувань/задач: ${warningCount}` : 'Активних нагадувань і задач немає.'}
              </p>
            </div>
          </div>
          {lastVisit && <button type="button" onClick={() => onRepeat(lastVisit)} className="w-full flex items-center justify-center gap-2 bg-slate-900 text-white font-black py-3 rounded-2xl hover:bg-slate-800 transition-colors"><History size={17} /> Повторити візит</button>}
        </div>
      </div>
    </div>
  );
...
function MiniStat({ label, value, danger, wide }) {
  return (
    <div className={`rounded-2xl border p-4 sm:p-3 min-w-0 ${danger ? 'bg-amber-50 border-amber-100' : 'bg-white border-slate-200'} ${wide ? 'col-span-2 lg:col-span-1' : ''}`}>
      <p className="text-[10px] sm:text-[9px] font-black uppercase text-slate-400 truncate">{label}</p>
      <p className="text-xl sm:text-lg font-black text-slate-900 truncate">{value}</p>
    </div>
  );
}
  ...
  return (
    <div className="bg-white rounded-3xl sm:rounded-2xl border border-slate-200 p-5 sm:p-4 shadow-sm min-w-0">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 sm:mb-3">
        <h3 className="text-base sm:text-sm font-black text-slate-800 flex items-center gap-2 min-w-0">{icon}<span className="break-words">{title}</span></h3>
        {action}
      </div>
      {children}
    </div>
...
function SmallValue({ label, value, wide }) {
  return (
    <div className={`bg-slate-50 border border-slate-100 rounded-2xl p-4 sm:p-3 min-w-0 ${wide ? 'sm:col-span-2' : ''}`}>
      <p className="text-[11px] sm:text-[10px] font-black uppercase text-slate-400">{label}</p>
      <p className="font-black text-slate-800 text-base sm:text-sm break-words">{value || '—'}</p>
    </div>
  );
}
  ...
  return (
    <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4 sm:p-3 min-w-0">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs sm:text-[11px] font-bold text-slate-500">
        ...
        {isNotDone(rec) && <button type="button" onClick={() => onDone(rec)} className="text-emerald-700 bg-emerald-50 border border-emerald-100 px-3 sm:px-2 py-2 sm:py-1 rounded-lg font-black uppercase flex items-center justify-center gap-1"><CheckCircle2 size={13} className="sm:w-3 sm:h-3" /> Виконано</button>}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4 sm:p-3 min-w-0">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs sm:text-[11px] font-bold text-slate-500">
        ...
        {isNotDone(task) && <button type="button" onClick={() => onDone(task)} className="text-emerald-700 bg-emerald-50 border border-emerald-100 px-3 sm:px-2 py-2 sm:py-1 rounded-lg font-black uppercase flex items-center justify-center gap-1"><CheckCircle2 size={13} className="sm:w-3 sm:h-3" /> Виконано</button>}
      </div>
    </div>
  );
}
...
function VisitMini({ visit, onRepeat }) {
  ...
  return (
    <div className="space-y-4 sm:space-y-3">
      ...
      <button type="button" onClick={() => onRepeat(visit)} className="w-full flex items-center justify-center gap-2 bg-slate-800 text-white font-bold py-3.5 sm:py-2.5 rounded-2xl sm:rounded-xl hover:bg-slate-900 transition-colors">
        <History size={17} className="sm:w-4 sm:h-4" /> Повторити візит
      </button>
    </div>
  );
}
  ...
  return (
    <div className="bg-slate-50 rounded-3xl sm:rounded-2xl p-5 sm:p-4 md:p-5 border border-slate-100 min-w-0">
      <div className="flex flex-col sm:flex-row sm:items-start justify-between mb-4 gap-3">
        <div className="min-w-0">
          <p className="text-base sm:text-sm font-black text-slate-700 flex items-center gap-2 break-words"><CalendarDays size={17} className="sm:w-4 sm:h-4" /> {formatVisitDate(visit)}</p>
      ...
      <div className="space-y-4 mb-4">
        <div className="bg-white rounded-2xl border border-slate-100 p-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-black text-slate-800 flex items-center gap-2"><Wrench size={16} className="text-blue-600" /> Послуги</h3>
          ...
          {services.length === 0 ? <p className="text-sm text-slate-400 font-semibold">Послуги не додані</p> : (
            <div className="space-y-2">
              {services.map((service, index) => (
                <div key={service.id || index} className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 rounded-xl bg-slate-50 px-3 py-2">
                  <p className="text-sm font-bold text-slate-700 leading-snug break-words">{getServiceName(service)}</p>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
            <h3 className="font-black text-slate-800 flex items-center gap-2"><Package size={16} className="text-emerald-600" /> Запчастини</h3>
            {articleCount > 1 && <CopyButton value={getAllArticles(visit)} label="Всі артикули" copiedLabel="Скопійовано" compact />}
          </div>
                ...
                return (
                  <div key={part.id || `${article}-${index}`} className="rounded-2xl border border-slate-200 bg-slate-50 p-3 min-w-0">
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 mb-2">
                      <div className="min-w-0">
                        {brand && <p className="text-[11px] font-black uppercase tracking-wide text-emerald-700 mb-1 break-words">{brand}</p>}
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-3">
                      {article ? <CopyButton value={article} label={article} copiedLabel="Скопійовано" title="Скопіювати артикул" compact /> : <span className="rounded-lg bg-slate-100 text-slate-400 px-2 py-1 text-xs font-bold">Артикул: —</span>}
                      {supplier && <span className="rounded-lg bg-white border border-slate-200 text-slate-500 px-2 py-1 text-xs font-bold break-words">{supplier}</span>}
                    </div>
                  </div>
                );
              })}
    ...
    <div className="fixed inset-0 z-[70] bg-slate-900/45 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4 overflow-hidden">
      <div className="w-[calc(100vw-1rem)] sm:w-full max-w-lg bg-white rounded-2xl shadow-xl overflow-hidden max-h-[calc(100dvh-1rem)] flex flex-col">
        <div className="p-5 bg-slate-50 border-b border-slate-100 flex items-center justify-between shrink-0">
          <h3 className="text-lg font-black text-slate-800 flex items-center gap-2"><ClipboardList size={18} /> Нова рекомендація</h3>
        ...
        <div className="p-5 space-y-3 overflow-y-auto overscroll-contain">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <input type="date" value={recommendationForm.due_date} onChange={(e) => setRecommendationForm((prev) => ({ ...prev, due_date: e.target.value }))} className="border border-slate-200 bg-slate-50 rounded-xl px-4 py-3 font-bold outline-none focus:border-blue-500" />
            <input type="number" value={recommendationForm.due_mileage} onChange={(e) => setRecommendationForm((prev) => ({ ...prev, due_mileage: e.target.value }))} placeholder="Пробіг" className="border border-slate-200 bg-slate-50 rounded-xl px-4 py-3 font-bold outline-none focus:border-blue-500" />
          </div>
        </div>
        <div className="p-5 bg-slate-50 border-t border-slate-100 flex flex-col sm:flex-row justify-end gap-2 shrink-0">
          <button type="button" onClick={onClose} className="px-4 py-3 rounded-xl bg-white border border-slate-200 font-black text-xs uppercase text-slate-600">Скасувати</button>
          <button type="button" onClick={submitRecommendation} className="px-4 py-3 rounded-xl bg-blue-600 text-white font-black text-xs uppercase hover:bg-blue-700">Зберегти</button>
        </div>
      </div>
    </div>
  );
...
function RepeatVisitModal({ repeatForm, setRepeatForm, submitRepeatVisit, onClose }) {
  return (
    <div className="fixed inset-0 z-[60] bg-slate-900/45 backdrop-blur-sm flex items-center justify-center p-4 overflow-hidden">
      <div className="w-[calc(100vw-1rem)] sm:w-full max-w-md bg-white rounded-2xl shadow-xl p-5">
        ...
        <div className="flex justify-end gap-2 mt-4">
          <button type="button" onClick={onClose} className="px-3 py-2 rounded-lg border">Скасувати</button>
