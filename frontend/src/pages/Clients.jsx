...
const Clients = ({ embedded = false }) => {
  ...
  return (
    <div className={embedded ? 'w-full max-w-full overflow-x-hidden' : 'w-full max-w-6xl mx-auto px-3 sm:px-4 md:px-8 py-4 md:py-8 min-h-screen bg-slate-50 overflow-x-hidden'}>
      ...
      {selectedGroup && <div className="fixed inset-0 z-50 flex items-stretch sm:items-center justify-center p-0 sm:p-4 bg-slate-900/50 backdrop-blur-sm overflow-hidden">
        <div className="bg-white w-full sm:w-full max-w-none sm:max-w-5xl rounded-none sm:rounded-3xl shadow-2xl overflow-y-auto sm:overflow-hidden flex flex-col h-[100dvh] sm:h-auto sm:max-h-[calc(100dvh-2rem)] min-w-0 overscroll-contain">
          ...
          <div className="px-4 sm:px-3 md:px-6 pt-4 bg-white border-b border-slate-100 overflow-x-auto sm:shrink-0">
            <div className="flex gap-2 min-w-max pb-3">
              {clientTabs.map((tab) => { const Icon = tab.icon; const active = detailTab === tab.key; return <button key={tab.key} type="button" onClick={() => setDetailTab(tab.key)} className={`flex items-center gap-2 px-4 sm:px-4 py-3 sm:py-2.5 rounded-2xl text-sm sm:text-xs font-black uppercase whitespace-nowrap transition-all ${active ? 'bg-blue-600 text-white shadow-md shadow-blue-100' : 'bg-slate-50 text-slate-500 hover:bg-slate-100'}`}><Icon size={16} className="sm:w-[15px] sm:h-[15px]"/>{tab.label}</button>; })}
            </div>
          </div>
          ...
          <div className="flex-none sm:flex-1 sm:min-h-0 p-4 sm:p-3 md:p-6 overflow-visible sm:overflow-y-auto sm:overscroll-contain space-y-5 sm:space-y-4 min-w-0 pb-8 sm:pb-6">
            {detailTab === 'overview' && <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 sm:gap-4">
              ...
              <InfoBlock title="Що треба не забути" icon={<ClipboardList size={18} className="text-amber-600"/>} action={<button type="button" onClick={openRecommendationModal} className="bg-blue-600 text-white px-4 sm:px-3 py-3 sm:py-2 rounded-xl text-xs sm:text-[10px] font-black uppercase flex items-center justify-center gap-1"><Plus size={14} className="sm:w-[13px] sm:h-[13px]"/> Додати</button>}>
              ...
              <InfoBlock title="Дані клієнта" icon={<FileText size={18} className="text-slate-600"/>}>
            ...
            {detailTab === 'recommendations' && <InfoBlock title="Рекомендації по клієнту" icon={<ClipboardList size={18} className="text-indigo-600"/>} action={<button type="button" onClick={openRecommendationModal} className="bg-blue-600 text-white px-4 sm:px-3 py-3 sm:py-2 rounded-xl text-xs sm:text-[10px] font-black uppercase flex items-center justify-center gap-1 hover:bg-blue-700"><Plus size={14} className="sm:w-[13px] sm:h-[13px]" /> Додати</button>}>
            ...
            {detailTab === 'tasks' && <InfoBlock title="Задачі по клієнту" icon={<ListChecks size={18} className="text-blue-600"/>}>
            ...
            {detailTab === 'visits' && <div className="space-y-5 sm:space-y-4">
...
function SmallValue({ label, value }) {
  return <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 sm:p-3 min-w-0"><p className="text-[11px] sm:text-[10px] font-black uppercase text-slate-400">{label}</p><p className="font-black text-slate-800 text-base sm:text-sm break-words">{value || '—'}</p></div>;
}

function RecommendationRow({ rec, onDone }) {
  return <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4 sm:p-3 min-w-0"><div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 mb-2 sm:mb-1"><p className="text-base sm:text-sm font-black text-slate-800 leading-snug break-words">{rec.title}</p><span className={`text-[10px] sm:text-[9px] font-black uppercase px-2 py-1 rounded-lg border shrink-0 self-start ${recBadge[rec.state] || recBadge.active}`}>{rec.state_label}</span></div>{rec.description && <p className="text-sm sm:text-xs font-semibold text-slate-500 mb-2 break-words">{rec.description}</p>}<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs sm:text-[11px] font-bold text-slate-500"><span>Дата: {formatDueDate(rec.due_date)} {rec.due_mileage ? `• ${rec.due_mileage} км` : ''}</span>{rec.status !== 'done' && <button type="button" onClick={() => onDone(rec)} className="text-emerald-700 bg-emerald-50 border border-emerald-100 px-3 sm:px-2 py-2 sm:py-1 rounded-lg font-black uppercase flex items-center justify-center gap-1"><CheckCircle2 size={13} className="sm:w-3 sm:h-3" /> Виконано</button>}</div></div>;
}

function TaskRow({ task, onDone }) {
  return <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4 sm:p-3 min-w-0"><div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 mb-2 sm:mb-1"><p className="text-base sm:text-sm font-black text-slate-800 leading-snug break-words">{task.title}</p><span className={`text-[10px] sm:text-[9px] font-black uppercase px-2 py-1 rounded-lg border shrink-0 self-start ${recBadge[task.state] || recBadge.new}`}>{task.state_label || task.status}</span></div>{task.description && <p className="text-sm sm:text-xs font-semibold text-slate-500 mb-2 break-words">{task.description}</p>}<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs sm:text-[11px] font-bold text-slate-500"><span>Дата: {formatDueDate(task.due_date)}</span>{task.status !== 'done' && <button type="button" onClick={() => onDone(task)} className="text-emerald-700 bg-emerald-50 border border-emerald-100 px-3 sm:px-2 py-2 sm:py-1 rounded-lg font-black uppercase flex items-center justify-center gap-1"><CheckCircle2 size={13} className="sm:w-3 sm:h-3" /> Виконано</button>}</div></div>;
}

function VisitMini({ visit, onRepeat }) {
  return <div className="space-y-4 sm:space-y-3"><div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-2"><SmallValue label="Дата" value={formatVisitDate(visit)} /><SmallValue label="Статус" value={visit.status || 'В обробці'} /></div><button type="button" onClick={() => onRepeat(visit)} className="w-full flex items-center justify-center gap-2 bg-slate-800 text-white font-bold py-3.5 sm:py-2.5 rounded-2xl sm:rounded-xl hover:bg-slate-900 transition-colors"><History size={17} className="sm:w-4 sm:h-4" /> Повторити візит</button></div>;
}
