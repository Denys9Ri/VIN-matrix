  ...
  return <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm p-3 sm:p-4 overflow-y-auto">
    <form onSubmit={onSubmit} className="bg-white rounded-[28px] w-full max-w-3xl mx-auto my-4 sm:my-8 shadow-2xl overflow-hidden">
      <div className="p-5 md:p-6 border-b border-slate-100 bg-slate-50/80 flex justify-between items-start gap-3">
        <div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-600 mb-1">CRM</p><h2 className="text-2xl font-black uppercase text-slate-900">Картка покупця</h2><p className="text-sm font-bold text-slate-500 mt-1">Дані клієнта і авто розділені на чисті блоки.</p></div>
...
function FormSection({ title, desc, children }) { return <section className="rounded-3xl border border-slate-200 bg-white p-4 md:p-5 shadow-sm space-y-3"><div><h3 className="font-black uppercase tracking-[0.16em] text-sm text-slate-900">{title}</h3>{desc && <p className="text-xs font-semibold text-slate-500 mt-1">{desc}</p>}</div>{children}</section>; }
function TextField({ label, value, onChange, placeholder }) { return <label className="block"><span className="block text-xs font-black uppercase text-slate-500 mb-1.5">{label}</span><textarea value={value || ''} onChange={(e)=>onChange(e.target.value)} placeholder={placeholder} className="w-full min-h-[92px] rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 font-bold outline-none focus:border-blue-500 focus:bg-white" /></label>; }
