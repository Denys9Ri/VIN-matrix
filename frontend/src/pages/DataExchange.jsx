...
import { Download, Upload, DatabaseBackup, FileSpreadsheet, Users, Package, ClipboardList, ShieldCheck } from 'lucide-react';
...
async function downloadFile(path, fallbackName) {
  try {
    ...
    alert('Не вдалося сформувати файл. Спробуйте ще раз.');
  }
}
...
export default function DataExchange() {
  ...
  const importClients = async (file) => {
    ...
    try {
      ...
      alert(e.response?.data?.error || 'Не вдалося імпортувати клієнтів.');
    } finally {
      setImporting(false);
    }
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-8 pb-24 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        <ExportCard icon={<Users/>} title="Клієнти" desc="ПІБ, телефон, кількість замовлень, суми, борги, статус клієнта." onClick={() => downloadFile('/api/export/clients/', 'vin_matrix_clients.csv')} />
        <ExportCard icon={<ClipboardList/>} title="Замовлення / візити" desc="ID, статус, клієнт, авто, VIN, суми, борг, доставка, коментар." onClick={() => downloadFile('/api/export/orders/', 'vin_matrix_orders.csv')} />
        <ExportCard icon={<Package/>} title="Склад" desc="Бренд, артикул, назва, залишок, закупка, продаж, постачальник." onClick={() => downloadFile('/api/export/inventory/', 'vin_matrix_inventory.csv')} />
        <ExportCard icon={<DatabaseBackup/>} title="Резервна копія" desc="JSON-копія клієнтів, замовлень, товарів, робіт і складу." onClick={() => downloadFile('/api/export/backup/', 'vin_matrix_backup.json')} dark />
      </div>
      ...
      <div className="bg-white rounded-[28px] border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div className="p-5 md:p-6 grid grid-cols-1 lg:grid-cols-3 gap-5 items-start">
          <div className="lg:col-span-2 rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-5">
            ...
            <button onClick={() => fileRef.current?.click()} disabled={importing} className="w-full min-h-[120px] rounded-2xl bg-white border border-slate-200 hover:border-blue-300 hover:bg-blue-50 transition flex flex-col items-center justify-center gap-3 text-center p-5">
...
function ExportCard({ icon, title, desc, onClick, dark }) {
  return <button onClick={onClick} className={`text-left rounded-[26px] p-5 border shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg ${dark ? 'bg-slate-900 border-slate-800 text-white' : 'bg-white border-slate-200 text-slate-900'}`}>
    ...
    <div className={`mt-4 flex items-center gap-2 text-xs font-black uppercase ${dark ? 'text-blue-200' : 'text-blue-600'}`}><Download size={15}/> Завантажити</div>
  </button>;
}
