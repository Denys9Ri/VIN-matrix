...
import { ClipboardList, FileSpreadsheet, History, KeyRound, Package, Plus, Search, Tag, Trash2, Truck, Upload, X } from 'lucide-react';
...
export default function Inventory() {
  ...
  const [supplierForm, setSupplierForm] = useState({ id: '', name: '', api_key: '', price_file: null, current: '' });
  ...
  const load = async () => {
    const [c, i, s, m] = await Promise.all([
  ...
  const addCategory = async (e) => { e.preventDefault(); await axios.post(`${API_BASE}/api/categories/`, { name: newCat }, { headers }); setNewCat(''); setModal(null); load(); };
  const delCategory = async (id) => { if (!confirm('Видалити категорію?')) return; await axios.delete(`${API_BASE}/api/categories/${id}/`, { headers }); if (String(cat) === String(id)) setCat(''); load(); };
  const delItem = async (id) => { if (!confirm('Видалити товар?')) return; await axios.delete(`${API_BASE}/api/inventory/${id}/`, { headers }); load(); };
  const delSupplier = async (id) => { if (!confirm('Видалити постачальника?')) return; await axios.delete(`${API_BASE}/api/suppliers/${id}/`, { headers }); load(); };
  ...
  return <div className="max-w-7xl mx-auto p-4 md:p-8 md:pl-72 min-h-screen overflow-x-hidden">
    ...
    <div className="bg-white border border-slate-200 rounded-3xl p-2 mb-5 shadow-sm overflow-x-auto"><div className="flex gap-2 min-w-max"><Tab active={tab==='stock'} onClick={()=>setTab('stock')} icon={<Package size={16}/>} label="Товари"/><Tab active={tab==='receipts'} onClick={()=>setTab('receipts')} icon={<ClipboardList size={16}/>} label="Прихід"/><Tab active={tab==='history'} onClick={()=>setTab('history')} icon={<History size={16}/>} label="Історія"/><Tab active={tab==='suppliers'} onClick={()=>setTab('suppliers')} icon={<Truck size={16}/>} label="Постачальники"/></div></div>
    {msg && <Notice text={msg} onClose={()=>setMsg('')}/>} 
...
function Panel({ children }) { return <div className="bg-white border border-slate-200 rounded-3xl p-4 md:p-6 shadow-sm min-w-0">{children}</div>; }
function Notice({ text, onClose }) { return <div className="mb-5 bg-blue-50 text-blue-700 border border-blue-100 rounded-2xl px-4 py-3 text-sm font-bold flex justify-between gap-3"><span>{text}</span><button onClick={onClose}><X size={16}/></button></div>; }
function Modal({ title, onClose, children, wide }) { return <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 overflow-y-auto p-4 flex items-start justify-center"><div className={`bg-white rounded-3xl w-full ${wide?'max-w-3xl':'max-w-md'} p-5 md:p-6 relative shadow-2xl my-8`}><button onClick={onClose} className="absolute right-4 top-4 bg-slate-100 rounded-full p-2 text-slate-400"><X size={18}/></button><h2 className="text-xl font-black uppercase text-slate-900 mb-5">{title}</h2>{children}</div></div>; }
