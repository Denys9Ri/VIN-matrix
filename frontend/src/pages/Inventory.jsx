...
import { ClipboardList, History, Package, Plus, Search, Tag, Trash2, Truck, X } from 'lucide-react';
...
export default function Inventory() {
  ...
  const fetchData = async () => {
    try {
      ...
      setMessage('Не вдалося завантажити склад.');
    }
  };
  ...
  return <div className="max-w-7xl mx-auto p-4 md:p-8 md:pl-72 min-h-screen overflow-x-hidden">
    <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4 mb-5">
      <div>
        ...
        <p className="text-slate-500 font-semibold text-sm mt-1">Залишки, прихід товару, історія руху та постачальники.</p>
      </div>
    ...
    {activeTab === 'stock' && <>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
        <Stat label="Товарів" value={items.length}/><Stat label="На суму" value={money(stockTotal)}/><Stat label="Мало залишку" value={lowStock}/><Stat label="Категорій" value={categories.length}/>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-5">
        <aside className="bg-white border border-slate-200 rounded-3xl p-4 shadow-sm h-fit">
          <div className="flex justify-between items-center mb-3"><h3 className="font-black uppercase text-xs text-slate-500 flex gap-2"><Tag size={15}/> Категорії</h3><button onClick={() => setIsCategoryModalOpen(true)} className="bg-blue-50 text-blue-600 rounded-xl p-2"><Plus size={15}/></button></div>
          <div className="flex lg:flex-col gap-2 overflow-x-auto pb-1">
            <CategoryBtn active={!selectedCategory} onClick={() => setSelectedCategory('')} label="Усі товари" />
            {categories.map((c) => <div key={c.id} className="flex items-center gap-1 shrink-0 lg:shrink"><CategoryBtn active={String(selectedCategory) === String(c.id)} onClick={() => setSelectedCategory(c.id)} label={c.name}/><button onClick={() => handleDeleteCategory(c.id)} className="text-slate-300 hover:text-red-500 p-2"><Trash2 size={14}/></button></div>)}
          </div>
        </aside>
        <main className="bg-white border border-slate-200 rounded-3xl p-4 shadow-sm min-w-0">
          <div className="flex flex-col md:flex-row gap-3 justify-between mb-4">
            <div className="relative w-full md:w-96"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16}/><input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Пошук по артикулу, бренду, назві..." className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-9 pr-4 py-3 text-sm font-bold outline-none"/></div>
            <button onClick={() => openReceipt()} className="bg-blue-600 text-white rounded-xl px-4 py-3 text-xs font-black uppercase flex items-center justify-center gap-2"><Plus size={15}/> Прихід</button>
          </div>
          <div className="hidden md:block overflow-x-auto"><table className="w-full text-left min-w-[720px]"><thead><tr className="border-b-2 border-slate-100"><Th>Артикул / бренд</Th><Th>Товар</Th><Th>К-сть</Th><Th>Закупка</Th><Th>Продаж</Th><Th></Th></tr></thead><tbody>{filteredItems.map((item)=><tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50"><td className="p-3"><p className="font-black text-slate-800">{item.article}</p><p className="text-[10px] font-black uppercase text-blue-600">{item.brand}</p></td><td className="p-3"><p className="font-bold text-slate-700">{item.name}</p><p className="text-[10px] font-bold text-slate-400 uppercase">{item.category_name || 'Без категорії'} {item.supplier_name ? `· ${item.supplier_name}` : ''}</p></td><td className="p-3"><span className="bg-slate-100 rounded-xl px-3 py-1 font-black">{item.quantity} шт</span></td><td className="p-3 font-black">{money(item.buy_price)}</td><td className="p-3 font-black text-blue-600">{money(item.sell_price)}</td><td className="p-3 text-right"><button onClick={()=>handleDeleteItem(item.id)} className="text-slate-300 hover:text-red-500"><Trash2 size={16}/></button></td></tr>)}{!filteredItems.length&&<tr><td colSpan="6" className="p-10 text-center text-slate-400 font-black uppercase">Товарів не знайдено</td></tr>}</tbody></table></div>
          <div className="md:hidden space-y-3">{filteredItems.map((item)=><StockCard key={item.id} item={item} onDelete={()=>handleDeleteItem(item.id)}/>)}</div>
        </main>
      </div>
    </>}

    {activeTab === 'receipts' && <div className="bg-white border border-slate-200 rounded-3xl p-4 md:p-6 shadow-sm"><div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-5"><div><h2 className="text-xl font-black uppercase text-slate-900">Прихід товару</h2><p className="text-sm font-semibold text-slate-500">Прийнятий товар збільшує залишок. Якщо артикул уже є — кількість додається.</p></div><button onClick={() => openReceipt()} className="bg-blue-600 text-white rounded-xl px-5 py-3 text-xs font-black uppercase flex justify-center gap-2"><Plus size={15}/> Новий прихід</button></div><ReceiptForm receipt={receipt} setReceipt={setReceipt} suppliers={suppliers} categories={categories} onSubmit={handleReceive}/></div>}
