...
const Partners = () => {
  ...
  const toggleClientAccess = async (client) => {
    try {
      await api.patch(`/api/platform-clients/${client.id}/`, { is_access_enabled: !client.is_access_enabled, payment_status: !client.is_access_enabled ? 'active' : 'inactive' });
      await loadData();
  ...
  const renewClient = async (client) => {
    ...
    try { await api.post(`/api/platform-clients/${client.id}/renew-30-days/`); await loadData(); }
