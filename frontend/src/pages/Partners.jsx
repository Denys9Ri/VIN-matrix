...
import { AlertTriangle, BadgeCheck, BadgeX, Copy, Eye, Power, RefreshCw, Search, ShieldCheck, Trash2, UserCheck, UserCog, Users } from 'lucide-react';
...
const Partners = () => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [partnersRes, clientsRes, alertsRes] = await Promise.all([
    ...
    <div className="max-w-7xl mx-auto p-3 md:p-8 min-h-screen">
      ...
      {alertList.length > 0 && (
        <div className="bg-orange-50 border border-orange-200 rounded-2xl p-4 mb-5">
          <div className="flex items-center gap-2 mb-3"><AlertTriangle className="text-orange-600" size={20} /><h3 className="font-black text-orange-800">Клієнти, у яких скоро закінчиться або вже закінчилась підписка</h3></div>
          <div className="grid md:grid-cols-2 gap-2">
            {alertList.map((item) => (
              <div key={item.id} className="bg-white/70 rounded-xl px-3 py-2 text-sm font-bold text-slate-700">
                {item.client_code_display} · {item.full_name} · до {item.subscription_end_display || '—'}
              </div>
            ))}
          </div>
        </div>
