...
const NOTIFICATION_CATEGORIES = [
  {
    ...
    description: 'Найближчі записи, прострочені візити та важливі нагадування по графіку.',
    icon: CalendarClock,
  },
  {
    key: 'status_updates',
    title: 'Статус авто та замовлень',
    description: 'Готовність авто, зміна етапу робіт та важливі зміни по замовленню.',
    icon: CarFront,
  },
  {
    key: 'payments',
    title: 'Борги та оплати',
    description: 'Незакриті суми, борги клієнтів та замовлення, які очікують оплату.',
    icon: WalletCards,
  },
  {
    key: 'inventory',
    title: 'Запчастини та склад',
    description: 'Низькі залишки, запчастини в очікуванні та події, що потребують уваги.',
    icon: PackageSearch,
  },
  {
    key: 'delivery',
    title: 'Доставка',
    description: 'Зміни статусу відправлень, прибуття та повернення доставки.',
    icon: Truck,
  },
  {
    key: 'crm',
    title: 'CRM та задачі',
    description: 'Задачі, сервісні нагадування та дії по клієнтах, які не варто пропустити.',
    icon: ClipboardCheck,
  },
];
...
export default function PushNotifications() {
  ...
  const togglePreference = async (key) => {
    ...
    try {
      const response = await api.patch('/api/push/preferences/', {
        preferences: { [key]: next },
      });
      if (response.data?.preferences) {
        setPreferences({ ...DEFAULT_PREFERENCES, ...response.data.preferences });
      }
    } catch (error) {
  ...
  return (
    <div className="mx-auto max-w-[1120px] space-y-5 px-3 py-5 md:px-6 md:py-8">
      <section className="overflow-hidden rounded-[32px] border border-slate-200 bg-white shadow-sm">
        <div className="bg-gradient-to-r from-slate-950 via-slate-900 to-blue-900 p-6 text-white md:p-8">
          <div className="flex items-start gap-4">
            <span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-white/10 text-blue-100 ring-1 ring-white/10">
              <BellRing size={27} />
            </span>
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm font-semibold leading-relaxed text-slate-300 md:text-base">
                Отримуйте важливі події по роботі навіть коли VIN Matrix закрита. Ви самі обираєте, які повідомлення потрібні.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-5 p-5 md:grid-cols-[1fr_auto] md:items-center md:p-7">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm font-semibold leading-relaxed text-slate-500">
              {enabled
                ? `На цьому пристрої все налаштовано. Обрано ${selectedCount} із ${NOTIFICATION_CATEGORIES.length} категорій.`
                : 'Увімкніть сповіщення на цьому пристрої, щоб не пропускати важливі події.'}
            </p>
          </div>
      ...
      {platform.isIos && !platform.standalone && (
        <InfoPanel tone="amber" icon={<Smartphone size={21} />} title="На iPhone відкрийте VIN Matrix з іконки">
          Для системних сповіщень iPhone програма має бути додана на початковий екран і запущена саме звідти.
        </InfoPanel>
      )}

      {permission === 'denied' && (
        <InfoPanel tone="rose" icon={<AlertTriangle size={21} />} title="Сповіщення заблоковані в налаштуваннях пристрою">
          Дозвольте сповіщення для VIN Matrix у системних налаштуваннях, після чого поверніться на цю сторінку.
        </InfoPanel>
      )}
      ...
      <section className={`rounded-[30px] border border-slate-200 bg-white p-4 shadow-sm md:p-6 ${!enabled ? 'opacity-60' : ''}`}>
        <div className="flex flex-col gap-2 border-b border-slate-100 pb-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h2 className="text-lg font-black text-slate-950 md:text-xl">Що надсилати</h2>
            <p className="mt-1 text-sm font-semibold text-slate-500">Зміни зберігаються автоматично для вашого акаунта.</p>
          </div>
          ...
          {NOTIFICATION_CATEGORIES.map(({ key, title, description, icon: Icon }) => (
            <div key={key} className="flex items-center gap-4 py-4 md:py-5">
              ...
              <div className="min-w-0 flex-1">
                <h3 className="text-sm font-black text-slate-950 md:text-base">{title}</h3>
                <p className="mt-1 text-xs font-semibold leading-relaxed text-slate-500 md:text-sm">{description}</p>
              </div>
              <PreferenceSwitch
                checked={Boolean(preferences[key])}
                disabled={!enabled || loading || Boolean(savingPreference)}
      ...
      <section className="overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-sm">
        ...
        >
          <div>
            <p className="text-sm font-black text-slate-800">Діагностика сповіщень</p>
            <p className="mt-0.5 text-xs font-semibold text-slate-400">Перевірка дозволу та доставки на цьому пристрої</p>
          </div>
        ...
        {showDiagnostics && (
          <div className="border-t border-slate-100 p-5">
            ...
            {enabled && (
              <div className="mt-4">
                <ActionButton onClick={sendDiagnostic} disabled={Boolean(busy)}>
                  {busy === 'test' ? <Loader2 className="animate-spin" size={17} /> : <Send size={17} />}
                  Надіслати перевірочне повідомлення
                </ActionButton>
              </div>
            )}
          </div>
...
function PreferenceSwitch({ checked, disabled, loading, onChange, label }) {
  return (
    <button
      ...
      onClick={onChange}
      disabled={disabled}
      className={`relative h-7 w-12 shrink-0 rounded-full transition disabled:cursor-not-allowed disabled:opacity-50 ${checked ? 'bg-blue-600' : 'bg-slate-300'}`}
    >
      <span className={`absolute top-1 flex h-5 w-5 items-center justify-center rounded-full bg-white shadow-sm transition-all ${checked ? 'left-6' : 'left-1'}`}>
...
function ActionButton({ children, onClick, disabled, primary = false }) {
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`inline-flex min-h-[44px] items-center justify-center gap-2 rounded-2xl px-5 text-xs font-black transition disabled:cursor-not-allowed disabled:opacity-50 ${
        primary
...
function InfoPanel({ tone, icon, title, children }) {
  ...
  return (
    <section className={`rounded-[24px] border p-5 shadow-sm ${classes}`}>
      <div className="flex items-start gap-3">
        <span className="mt-0.5 shrink-0">{icon}</span>
        <div>
          <h2 className="text-sm font-black">{title}</h2>
          <p className="mt-1.5 text-sm font-semibold leading-relaxed opacity-80">{children}</p>
        </div>
      </div>
    </section>
  );
