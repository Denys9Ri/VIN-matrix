...
export default function Visits() {
  ...
  const [newVisitData, setNewVisitData] = useState({ plate: '', vin_code: '', client: '', phone: '', date: dateISO(), time: '', delivery_type: 'pickup', delivery_data: '', payment_status: 'unpaid', prepayment_amount: '', ...emptyCarData });
...
function Summary({visit,recommendations,workflowInfo,editComment,setEditComment,onSave}){const c=partCounts(visit);const activeRecs=recommendations.filter(r=>r.status!=='done'&&r.status!=='cancelled');const acceptanceDone=workflowFilled(workflowInfo.acceptance,'acceptance');const diagnosticDone=workflowFilled(workflowInfo.diagnostic,'diagnostic');const notReady=[];if(c.WAITING)notReady.push(`${c.WAITING} запчастин очікує замовлення`);if(c.IN_TRANSIT)notReady.push(`${c.IN_TRANSIT} запчастин в дорозі`);if(activeRecs.length)notReady.push(`${activeRecs.length} активних рекомендацій`);if(!acceptanceDone)notReady.push('акт приймання не заповнений');if(!diagnosticDone)notReady.push('діагностика не заповнена');const canClose=c.WAITING===0&&c.IN_TRANSIT===0;return <div className="space-y-4"><div className="grid grid-cols-1 md:grid-cols-4 gap-3"><InfoCard label="Роботи" value={`${servicesOf(visit).length} · ${money(servicesTotal(visit))}`}/><InfoCard label="Запчастини" value={`${partsOf(visit).length} · ${money(partsTotal(visit))}`}/><InfoCard label="Рекомендації" value={activeRecs.length}/><InfoCard label="Разом" value={money(totalOf(visit))}/></div><div className={`rounded-2xl border p-4 ${canClose?'bg-emerald-50 border-emerald-100':'bg-amber-50 border-amber-100'}`}><p className={`text-sm font-black uppercase ${canClose?'text-emerald-700':'text-amber-700'}`}>{canClose?'Можна закривати по запчастинах':'Є що проконтролювати'}</p>{notReady.length>0&&<ul className="mt-2 space-y-1 text-sm font-bold text-slate-700">{notReady.map((x,i)=><li key={i}>• {x}</li>)}</ul>}</div><div className="grid grid-cols-1 md:grid-cols-2 gap-3"><InfoCard label="Акт приймання" value={acceptanceDone?'Заповнений':'Не заповнений'}/><InfoCard label="Діагностика" value={diagnosticDone?'Заповнена':'Не заповнена'}/></div><textarea value={editComment} onChange={(e)=>setEditComment(e.target.value)} className="w-full bg-amber-50 border border-amber-100 rounded-xl p-3 text-sm font-bold min-h-[100px]" placeholder="Внутрішній коментар"/><button onClick={onSave} className="bg-amber-400 text-amber-950 rounded-xl px-5 py-3 text-xs font-black uppercase">Зберегти внутрішній коментар</button></div>}
